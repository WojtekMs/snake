...
void SnakeBoard::set_normal_mode_obstacles()
{
    ...
    int obstacle_1_left_horizontal_indent =
        (width - 4 - distance_between_obstacles_1) / 2;
    ...
    for (int i = 0; i < obstacle_1_size; ++i) {
        board_obstacles[i + obstacle_1_vertical_indent]
                       [obstacle_1_left_horizontal_indent + 1] = true;
        board_obstacles[i + obstacle_1_vertical_indent]
                       [width - 2 - obstacle_1_right_horizontal_indent] = true;
    }

    int obstacle_2_outer_denting = distance_between_obstacles_1 / 2 * 0.1111;  // ---  ---
    obstacle_2_outer_denting =
        (obstacle_2_outer_denting < 1) ? 1 : obstacle_2_outer_denting;
    int obstacle_2_inner_denting = distance_between_obstacles_1 / 2 * 0.2222;
    obstacle_2_inner_denting =
        (obstacle_2_inner_denting < 2) ? 2 : obstacle_2_inner_denting;
    int obstacle_2_size = (distance_between_obstacles_1 - 2 * obstacle_2_outer_denting -
                           obstacle_2_inner_denting) /
                          2;

    for (int i = 0; i < obstacle_2_size; ++i) {
        board_obstacles[height / 2 - 1][i + obstacle_1_left_horizontal_indent + 1 +
                                        obstacle_2_outer_denting + 1] = true;
        board_obstacles[height / 2 - 1][width - 2 - obstacle_1_right_horizontal_indent -
                                        obstacle_2_outer_denting - 1 - i] = true;
    }
}
...
void SnakeBoard::set_hard_mode_obstacles()
{
    ...
    for (int i = 0; i < distance_between_obstacles_1 / 2 + 1; ++i) {
        board_obstacles[obstacle_1_vertical_indent]
                       [2 * i + obstacle_1_left_horizontal_indent + 2] = true;
        board_obstacles[obstacle_1_vertical_indent + obstacle_1_size - 1]
                       [2 * i + obstacle_1_left_horizontal_indent + 2] = true;
    }
}
...
bool SnakeBoard::out_of_range(int col, int row) const
{
    if (col == 0 || col == width - 1) {
        if (std::find(vertical_gate_y.begin(), vertical_gate_y.end(), row) !=
            vertical_gate_y.end()) {
            return false;
        }
    }
    if (row == 0 || row == height - 1) {
        if (std::find(horizontal_gate_x.begin(), horizontal_gate_x.end(), col) !=
            horizontal_gate_x.end()) {
            return false;
        }
    }
    if (col < 0 || col > width - 1)
...
char SnakeBoard::get_tile_info(int col, int row) const
{
    ...
    if (board_obstacles[row][col])
        return 'X';
    if (row == 0 && std::find(horizontal_gate_x.begin(), horizontal_gate_x.end(), col) !=
                        horizontal_gate_x.end())
        return 'T';
    if (row == height - 1 &&
        std::find(horizontal_gate_x.begin(), horizontal_gate_x.end(), col) !=
            horizontal_gate_x.end())
        return 'B';
    if (col == 0 && std::find(vertical_gate_y.begin(), vertical_gate_y.end(), row) !=
                        vertical_gate_y.end())
        return 'L';
    if (col == width - 1 &&
        std::find(vertical_gate_y.begin(), vertical_gate_y.end(), row) !=
            vertical_gate_y.end())
        return 'R';
    return ' ';
}
