...
{
    if (!wall_texture.loadFromFile("textures/wall_texture.png")) {
        std::cerr << strerror(errno) << std::endl;
        abort();
    }
    if (!grass_texture.loadFromFile("textures/grass_texture.png")) {
        std::cerr << strerror(errno) << std::endl;
        abort();
    }
    if (!food_texture.loadFromFile("textures/apple_texture_2.png")) {
        std::cerr << strerror(errno) << std::endl;
        abort();
    }
    if (!snake_body_texture.loadFromFile("textures/body_texture.png")) {
        std::cerr << strerror(errno) << std::endl;
        abort();
    }
    if (!snake_head_texture.loadFromFile("textures/head_texture.png")) {
        std::cerr << strerror(errno) << std::endl;
        abort();
    }
...
void Game::draw_end_game_screen(sf::RenderWindow& window)
{
    ...
    text.setPosition(
        end_game_textbox.getPosition().x +
            (end_game_textbox.getSize().x - text.getLocalBounds().width) / 2,
        end_game_textbox.getPosition().y +
            (end_game_textbox.getSize().y - text.getLocalBounds().height) / 2);
